...
function Get-LatestRelease {
    # Returns the HIGHEST-versioned release for a plugin, not the first one the
    # CLI happens to list. `gh release list` sorts by publish/create date, not by
    # version, so editing an older release (which bumps its date) or a different
    # gh version's sort order could otherwise float an older tag to the top and
    # get it installed over a newer one. We scan every match and keep the max.
    # The version pattern accepts 2- to 4-part tags (V2.1, V2.0.3, V2.0.3.1).
    param([string]$TagPrefix)
    try {
        $releases = cmd /c ('gh release list --repo ' + $GithubRepo + ' --limit 200 2>nul')
        if ($LASTEXITCODE -ne 0) { return $null }

        $bestVer = $null
        $bestRaw = $null
        foreach ($line in $releases -split "`n") {
            if ($line -match "$TagPrefix`_V(\d+(?:\.\d+){1,3})") {
                $raw = $Matches[1]
                $v = $null
                try { $v = [Version]$raw } catch { continue }
...
function ConvertTo-Version4 {
    # Parses a 1- to 4-part version string into a [Version] padded to 4 parts,
    # so comparisons are apples-to-apples no matter how many components each
    # side has. Installed DLLs report 4-part FileVersions (e.g. 2.0.3.0) while
    # release tags may be 2- to 4-part (2.1, 2.0.3, 2.0.3.1); without padding,
    # [Version]2.0.3.0 compares GREATER than [Version]2.0.3 (revision 0 > -1)
    # and every up-to-date install would look outdated.
    param([string]$Text)
...
function Compare-Versions {
    ...
    $instVer = ConvertTo-Version4 $Installed
    $latVer  = ConvertTo-Version4 $Latest
    if ($null -eq $instVer -or $null -eq $latVer) { return $false }
